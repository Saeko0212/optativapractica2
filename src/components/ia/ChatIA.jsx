...
// Inicializar la instancia de Gemini fuera del componente para evitar recrearla en cada renderizado
...
const ChatIA = ({ mostrar, onCerrar }) => {
  ...
  const enviarConsulta = async () => {
    ...
    try {
      ...
      - Usa alias claros cuando hagas JOIN o agrupaciones.
      - REGLA VITAL: Si el usuario pregunta por "ventas", "total de ventas" o similares, asume que puede haber ambigüedad. Devuelve SIEMPRE dos columnas: una con el conteo de transacciones (ej. COUNT(id_venta) AS cantidad_de_ventas) y otra con la suma del dinero (ej. SUM(total) AS total_dinero_recaudado).
      ...
      // 1. Limpiamos las etiquetas de Markdown de forma segura (solo borramos la etiqueta, no el contenido)
      texto = texto.replace(/```json/gi, '').replace(/```sql/gi, '').replace(/```/g, '').trim();
      ...
      if (inicioJson === -1 || finJson === 0) {
        throw new Error("No se pudo encontrar la estructura JSON en la respuesta de la IA.");
      }

      // 3. Aislamos el JSON puro y lo convertimos
      const jsonString = texto.substring(inicioJson, finJson);
      const respuestaIA = JSON.parse(jsonString);
      ...
      setMensajes(prev => [...prev, {
        tipo: 'ia',
        explicacion: "No entendí bien tu consulta o hubo un problema al recuperar los datos. Por favor, reformúlala de forma clara.",
        error: true
      }]);
    }
