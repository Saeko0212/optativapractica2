...
const Ventas = () => {
      ...
      const { data, error } = await supabase
        ...
        .select(`
          *,
          ...
          detalles_ventas (*, productos (nombre, nombre))
        `)
  ...
  useEffect(() => {
    if (ventaAEditar) {
      // Uso de comodines por si la columna está en singular o plural en la base de datos
      const idBuscado = ventaAEditar.id_cliente || ventaAEditar.id_clientes;
      const cliente = clientes.find(c => (c.id_cliente || c.id_clientes) === idBuscado);
      ...
      if (ventaAEditar.detalles_ventas?.length > 0) {
        const detallesFormateados = ventaAEditar.detalles_ventas.map(d => ({
          id_productos: d.id_producto || d.id_productos,
          nombre: d.productos?.nombre || d.productos?.nombre_producto || "Producto",
  ...
  const agregarDetalle = (producto, cantidad) => {
    ...
    setDetalles((prev) => {
      const idProd = producto.id_productos || producto.id_producto;
      const existe = prev.find((d) => (d.id_productos || d.id_producto) === idProd);
      
      if (existe) {
        return prev.map((d) =>
          (d.id_productos || d.id_producto) === idProd
  ...
  const eliminarDetalle = (id_productos) => {
    setDetalles(prev => prev.filter(d => (d.id_productos || d.id_producto) !== id_productos));
  };

  const actualizarCantidad = (id_productos, nuevaCantidad) => {
    if (nuevaCantidad < 1) return;
    setDetalles(prev => prev.map(d =>
      (d.id_productos || d.id_producto) === id_productos ? { ...d, cantidad: nuevaCantidad } : d
    ));
  };
  ...
  const guardarVenta = async () => {
    ...
    // Generamos la hora exacta
    const nicaNow = () => new Date().toLocaleString("sv", { timeZone: "America/Managua" }).replace(" ", "T");

    try {
      if (ventaAEditar) {
        // === ACTUALIZAR ===
        const { data: updateData, error: updateError } = await supabase.from("ventas").update({
          id_clientes: clienteSeleccionado.id_cliente || clienteSeleccionado.id_clientes, // <-- LLAVE CORREGIDA (con S)
          id_empleado: empleadoSeleccionado.id_empleado,
          metodo_pago: metodoPago,
          total: totalGeneral,
          fecha_venta: nicaNow() 
        }).eq("id_venta", ventaAEditar.id_venta).select(); 

        if (updateError) throw updateError;
        
        if (!updateData || updateData.length === 0) {
          throw new Error("Supabase bloqueó la actualización (Fallo Silencioso por RLS). Por favor, asegúrate de haber ejecutado los comandos DISABLE ROW LEVEL SECURITY en el SQL Editor.");
        }
        ...
        const detallesInsert = detalles.map(d => ({
          ...
          id_productos: d.id_productos || d.id_producto, // <-- LLAVE CORREGIDA (con S)
          cantidad: d.cantidad,
          precio_unitario: d.precio,
          subtotal: d.cantidad * d.precio
        }));

        const { error: detallesError } = await supabase.from("detalles_ventas").insert(detallesInsert);
        ...
        const { error: errorInsertarDetalles } = await supabase.from("detalles_ventas").insert(detallesInsert);
        if (errorInsertarDetalles) throw errorInsertarDetalles;
      ...
      const mensajeError = typeof err === 'object' ? JSON.stringify(err) : String(err);
      setToast({ mostrar: true, mensaje: "Falló: " + mensajeError, tipo: "error" });
    }
  };
